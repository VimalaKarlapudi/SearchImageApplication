...
export class SerchImageComponent implements OnInit {
  ...
  images:IImage[];
  constructor(private keywordService:KeywordService,private searchImageService:SearchImageService,private myRoute:Router){
  ...
  searchImage(searchForm:FormGroup){
    if(searchForm.valid){
      ...
      return this.searchImageService.getImages(searchForm.value.word).subscribe(data=>this.display(data),
